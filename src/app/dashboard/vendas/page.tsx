...
export default async function VendasPage({
  ...
  // "Tabela atacado" nunca aparece nem é selecionável aqui — pedido do Rodrigo em 2026-09-24/25:
  // atacado fica 100% isolado na aba Atacado, sem misturar nos números/filtro do resto do Radar
  // (ver getTabelaPrecoRestrictionSemAtacado em lib/permissions.ts).
  ...
  // Querystring de loja/marca/tabelaPreco pra repassar pro fetch client-side do drilldown de
  // produto (/api/vendas/produtos-familia) — mesmos filtros da página, sem os demais params
  // (dimension, filtros=1 etc, que não fazem sentido nessa rota).
  const filtrosQueryParams = new URLSearchParams();
  for (const id of filters.storeIds ?? []) filtrosQueryParams.append("store", id);
  for (const m of filters.marcas ?? []) filtrosQueryParams.append("marca", m);
  for (const t of filters.tabelasPreco ?? []) filtrosQueryParams.append("tabelaPreco", t);
  ...
  return (
    <div>
      ...
      <FamiliaProdutoSection porFamilia={porFamilia} filtrosQuery={filtrosQuery} />
